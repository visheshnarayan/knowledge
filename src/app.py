...
def create_app(parent_agent, child_agents, graph_paths, initial_search_similarity):
    ...
    def index():
        ...
        return render_template(
            "index.html", initial_search_similarity=initial_search_similarity
        )
    ...
    def update_search_similarity():
        ...
        for agent_name, agent_instance in child_agents.items():
            ...
            logger.info(
                f"Updated search_similarity for agent '{agent_name}' to {new_similarity}."
            )
    ...
    def query():
        ...
        # If a list of topics is returned, join them for display
        if isinstance(routed_topics, list):
            routed_topic_str = ", ".join(routed_topics)
        else:
            routed_topic_str = routed_topics

        logger.info(
            f"Returning response from parent agent. Routed topic(s): {routed_topic_str}"
        )
        return jsonify(
            {"response": response, "sources": sources, "routed_topic": routed_topic_str}
        )
