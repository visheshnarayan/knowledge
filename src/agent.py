...
class Agent:
    def __init__(self, subgraph, topic, embedding_model, embedding_tokenizer, search_similarity, search_sample_ratio, strands_config, child_agent_model_id, debug_config=None):
        ...
        logger.info(f"Connecting to AWS Bedrock for Strands API for child agent '{topic}'...")
        llm = BedrockModel(model_id=child_agent_model_id)
        
        self.agent = StrandsAgent(
            system_prompt=self.strands_config['agent']['system_prompt'],
            model=llm,
            tools=[self.similarity_search_tool]
        )

        logger.info(f"Strands agent for topic '{self.topic}' initialized with model {child_agent_model_id}.")
    ...
    @classmethod
    def from_state(cls, state, embedding_model, embedding_tokenizer, child_agent_model_id, debug_config=None):
        return cls(
            state['subgraph'],
            state['topic'],
            embedding_model,
