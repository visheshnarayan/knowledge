...
class GraphBuilder:
    def __init__(self, data_config, graph_config):
        ...
        self.embedding_tokenizer = AutoTokenizer.from_pretrained(self.graph_config['embedding']['model'])
        self.embedding_model = AutoModel.from_pretrained(self.graph_config['embedding']['model'])
        ...
        self.triplet_tokenizer = AutoTokenizer.from_pretrained(self.graph_config['triplet_extraction']['model'])
        self.triplet_model = AutoModelForSeq2SeqLM.from_pretrained(self.graph_config['triplet_extraction']['model'])
    ...
    def build(self):
        ...
        print(f"Cosine graph built with {self.cosine_graph.number_of_nodes()} nodes and {self.cosine_graph.number_of_edges()} edges.")
        print(f"Triplets graph built with {self.triplets_graph.number_of_nodes()} nodes and {self.triplets_graph.number_of_edges()} edges.")
        ...
        for filename in os.listdir(input_dir):
            if filename.endswith(".txt"):
                ...
                for i, chunk in enumerate(chunks):
                    ...
                    self.cosine_graph.add_node(node_id, content=chunk, type="chunk", filename=filename)
                    self.triplets_graph.add_node(node_id, content=chunk, type="chunk", filename=filename)
    ...
    def _embed_nodes(self):
        ...
        for node_id, node_data in self.cosine_graph.nodes(data=True):
            if node_data['type'] == "chunk":
                ...
                inputs = self.embedding_tokenizer(node_data['content'], return_tensors="pt", truncation=True, padding=True)
                with torch.no_grad():
    ...
    def _connect_nodes(self):
        ...
        if self.similarity_metric != 'cosine':
            raise ValueError(f"Similarity metric '{self.similarity_metric}' is not supported. Please use 'cosine'.")

        chunk_nodes = [n for n in self.cosine_graph.nodes(data=True) if n[1]['type'] == "chunk"]
        for i in range(len(chunk_nodes)):
            for j in range(i + 1, len(chunk_nodes)):
                ...
                if similarity > self.similarity_threshold:
                    self.cosine_graph.add_edge(node1_id, node2_id, weight=similarity, type="semantic_similarity")

    def _extract_triplets(self):
        print("Extracting triplets...")
        # Collect chunk nodes first to avoid RuntimeError during iteration
        chunk_nodes_to_process = [(node_id, node_data) for node_id, node_data in self.triplets_graph.nodes(data=True) if node_data['type'] == "chunk"]

        for node_id, node_data in chunk_nodes_to_process:
            text = node_data['content']
            try:
                # Prepare input for REBEL model
                model_inputs = self.triplet_tokenizer(text, max_length=256, truncation=True, return_tensors="pt")
                ...
                decoded_triplets = self.triplet_tokenizer.batch_decode(generated_tokens, skip_special_tokens=False)[0]
                # Remove <s> and </s> tokens
                cleaned_triplets = decoded_triplets.replace("<s>", "").replace("</s>", "").strip()
                ...
                for i, triplet in enumerate(parsed_triplets):
                    ...
                    self.triplets_graph.add_node(triplet_node_id, content=f"({subject}, {predicate}, {obj})", type="triplet")
                    self.triplets_graph.add_edge(node_id, triplet_node_id, type="contains_triplet", triplet_info=f"({subject}, {predicate}, {obj})")
